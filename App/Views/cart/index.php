  ...
  <form action="<?= DOCUMENT_ROOT ?>/cart/checkout" method="post">
    <div class="cart-detail">
        ...
        <?php foreach ($data['booksInCart'] as $key => $book) : ?>
          <li class="cart-detail__book__item">
            ...
            <div class="cart-detail__book__item__info">
              ...
              <label class="cart-detail__book__item__info__number" for="numOfBook<?= $book['MSHH'] ?>">Số lượng: <input type="number" value="1" min="1" max="99" name="numOfBook[]"></label>
            </div>
          </li>
        <?php endforeach; ?>
      </ul>
      <div class="cart-detail__user-info">
        ...
        <?php if (isset($_SESSION['user'])) : ?>
          <div class="cart-detail__user-info__detail">
            ...
            <div class="cart-detail__user-info__detail__total">
              <b>Tổng tiền:</b>
              <b class="price"> <?= number_format($data['total'], 0) ?>đ</b>
            </div>
            <div class="cart-detail__user-info__detail__total">
        ...
        <?php else : ?>
          <p>Bạn chưa đăng nhập, hãy đăng nhập để đặt hàng!</p>
        <?php endif; ?>
      </div>
    </div>
  </form>
