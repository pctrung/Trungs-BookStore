     ...
     <div class="row mb-2">
       ...
       <div class="col-sm-6">
         <ol class="breadcrumb float-sm-right">
           ...
           <li class="breadcrumb-item active">Chỉnh sửa đơn đặt hàng</li>
         </ol>
       </div>
     </div>
   </div>
 ...
 <section class="content">
   ...
   <form onsubmit="return orderValidate();" action="<?= DOCUMENT_ROOT ?>/admin/order/update/<?= $data['updateOrder']['SoDonDH'] ?>" method="post" enctype="multipart/form-data">
     <div class="row">
       <div class="col-md-12">
         <div class="card card-primary">
           <div class="card-header">
             <h3 class="card-title">Chỉnh sửa đơn đặt hàng</h3>
           </div>
           <div class="card-body">
             <div class="form-group">
               <label for="MSNV">Số đơn đặt hàng</label>
               <input readonly type="text" id="SoDonDH" name="SoDonDH" class="form-control" value="<?= $data['updateOrder']['SoDonDH'] ?>"></input>
             </div>
             <div class="form-group">
             ...
             <div class=" form-group">
               ...
               <select required class="form-control custom-select" name="TrangThai" id="TrangThai">
                 ...
                 <option value="Chưa xử lý" <?= $data['updateOrder']['TrangThai'] == 'Chưa xử lý' ? 'selected' : '' ?>>Chưa xử lý</option>
                 <option value="Đang chuẩn bị hàng" <?= $data['updateOrder']['TrangThai'] == 'Đang chuẩn bị hàng' ? 'selected' : '' ?>>Đang chuẩn bị hàng</option>
                 <option value="Đang giao hàng" <?= $data['updateOrder']['TrangThai'] == 'Đang giao hàng' ? 'selected' : '' ?>>Đang giao hàng</option>
                 <option value="Đã giao" <?= $data['updateOrder']['TrangThai'] == 'Đã giao' ? 'selected' : '' ?>>Đã giao</option>
               </select>
             </div>
             <div class="form-group">
               <label for="numOfBook">Số lượng sách</label>
               <div class="input-group">
                 <input onchange="onNumOfBookChange(this)" required type="number" id="numOfBookInput" name="numOfBookInput" class="form-control" value="<?= $data['updateOrder']['SoLuongSachMua'] ?>" min="1" max="99"></input>
             ...
             <div id="bookOption">
               <?php foreach ($data['updateOrder']['DonHang'] as $key => $order) : ?>
                 <div class="form-group">
                   <h4 class="font-weight-bold text-info">Sách <?= $key + 1 ?></h4>
                   <div class="row">
                     <div class="col-6"><label for="bookOption<?= $key + 1 ?>ID">Mã sách <?= $key + 1 ?></label><input type="text" id="bookOption<?= $key + 1 ?>ID" name="MSHH[]" class="form-control" required value=<?= $order['MSHH'] ?>></div>
                     <div class="col-6"><label for="bookOption<?= $key + 1 ?>Number">Số lượng</label><input type="number" id="bookOption<?= $key + 1 ?>Number" name="SoLuong[]" class="form-control" required value=<?= $order['SoLuong'] ?>></div>
                   </div>
                   <div id="bookOption<?= $key + 1 ?>Detail" class="text-info mt-1">
                   </div>
                 </div>
               <?php endforeach; ?>
             </div>
             <div class="form-group">
               <label for="ThanhTien">Tổng tiền: <span id="ThanhTien" class="text-success">
                   <?= number_format($data['updateOrder']['ThanhTien'], 0, '', ','); ?> VND</span></label>
 ...
 <script>
   ...
   function initEditOrderPage() {
     var i = 1;
     while (document.getElementById("bookOption" + i + "ID")) {

       var option = document.getElementById("bookOption" + i + "ID");
       if (option.value != "") {
         ...
         ajax.onreadystatechange = function() {
           if (this.readyState == 4 && this.status == 200) {
             if (this.responseText == "false") {
               ...
               bookFound(option, this.responseText);
             }
           }
         };

         ajax.open("post", "<?= DOCUMENT_ROOT ?>/admin/book/getByIDJSON/" + option.value + "/" + <?= $data['updateOrder']['SoDonDH'] ?>, false);
         ajax.send();
     ...
     message.innerHTML = `
       <div class="row">
        ...
        <div class="col-10">
            ...
            <div>
              Giảm giá: <input name="GiamGia[]" class="text-dark form-control" value="${book.GiamGia == undefined ? "" : book.GiamGia}"></input>
            </div>
        </div>
       </div>
     `
 ...
 <script>
   <?php foreach ($data['updateOrder']['DonHang'] as $key => $order) : ?>
     document.getElementById("bookOption<?= $key + 1 ?>ID").addEventListener('input', onChangeBookID);
   <?php endforeach; ?>
 </script>
 <!-- /.content -->